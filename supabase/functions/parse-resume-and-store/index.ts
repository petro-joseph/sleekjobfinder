...
import type { ParsedResumeDbData, ParsedPersonalData, ParsedEducation, ParsedExperience } from './interfaces/resume.ts';
...
async function extractTextFromFile(fileBuffer: ArrayBuffer, fileName: string): Promise<string | null> {
...
async function parseWithSuperParser(fileBuffer: ArrayBuffer, fileName: string, apiKey: string): Promise<ParsedResumeDbData | null> {
    ...
    try {
        ...
        const mappedData: ParsedResumeDbData = {
            ...
            education: (parsed.education_history || []).map((edu: any) => ({
...
async function parseWithOpenAI(text: string, apiKey: string): Promise<ParsedResumeDbData | null> {
...
function parseInternally(text: string): ParsedResumeDbData {
    console.log('Attempting internal basic parsing');
    const personal: ParsedPersonalData = {
        full_name: null,
        email: text.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/gi)?.[0] || null,
        phone: text.match(/(\+\d{1,3}\s?)?($$?\d{3}$$?[\s.-]?)?\d{3}[\s.-]?\d{4}/g)?.[0] || null,
    ...
    const skillsKeywords = ['javascript', 'react', 'node', 'python', 'java', 'sql', 'aws', 'docker', 'kubernetes', 'typescript'];
...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (fetchError || !resumeRecord) {
            ...
            return new Response(JSON.stringify({ error: `Resume not found or fetch error: ${fetchError?.message}` }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 404,
            });
        }
        ...
        if (!resumeRecord.file_path || !resumeRecord.name) {
            ...
            return new Response(JSON.stringify({ error: 'Resume record incomplete (missing file_path or name).' }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 400,
            });
        }
