...
export const JobsMetadata = ({ filters, totalJobs }: JobsMetadataProps) => {
  ...
  const generateTitle = (): string => {
    ...
    if (parts.length === 0) {
      return "Find Your Perfect Job | KaziHub";
    }

    return `${parts.join(' ')} Jobs | KaziHub`;
  };
  ...
  return (
    <Helmet>
      ...
      {/* Canonical URL */}
      <link rel="canonical" href={`https://KaziHub.com/jobs${filters.searchTerm ? `?q=${filters.searchTerm}` : ''}`} />
    </Helmet>
  );
};
